...
from perplexity import *  # NOQA
from utility_gpt import *  # NOQA
...
def sample_sentence_noguide(text, this_sequence, tokenizer, model, prev_proba=1, top_k=0, top_p=0.9, temperature=1.0, eos_c=0, det_BS=False, ith=0):
...
def conditional_language_generation(
    ...
    if mode == "best_tour":
        best_order = best_tour(keywords_enc)  # NOQA
    ...
    # np.random.seed(seed)
    weight = constant
    converter_table = np.load(str(os.path.dirname(os.path.abspath(__file__))) + "/data/converter_table_" + str(embedding) + ".npy")
            ...
            for i in range(number_of_concurrent_sentences):
                ...
                if guide[b]:
                    ...
                    for j in range(number_of_words_per_sentence):
                        context, guide_words, guide_next, guide_probs, proba, this_sequence, c_time, t_time = sample_sentence(
                            context,
                            this_sequence,
                            tokenizer,
                            model,
                ...
                if type(proba) == torch.Tensor:  # NOQA
                    proba = proba.item()

                score_ = get_score(k, number_of_words_per_sentence, online_probability[b], proba)
                w_c = number_keywords - len(guide_words)
                if not no_do_wc:
                    quality_score = evaluate_quality_linear(this_sequence, w_c, score_)  # NOQA
                else:
                    quality_score = evaluate_quality_linear(this_sequence, 0, score_)  # NOQA
                ...
                # Linear Q
                result_subsequences.append([context, quality_score, w_c, score_, online_probability[b] * proba, guide_words, guide[b], eos_c, guide_probs, t_time, c_time])
    ...
    distilGPT2_perplexity = distilGPT2_perplexity_score(full_text[0])  # NOQA
    ...
    for b in range(number_of_beams):
        ...
        text_file_sentences.write("\nPerplexity: " + str(distilGPT2_perplexity_score(full_text[b])))  # NOQA
    text_file_sentences.write("\n###############################\n")
...
def get_args(parser):
    ...
    parser.add_argument("-mode", type=str, default="max", choices=["max", "next", "all", "random", "best_tour"], help="modes: max, next, all, random, best_tour")
    ...
    parser.add_argument("-task", type=str, default="50keywords", choices=["50keywords", "ROC", "key2article", "commongen"], help="tasks: 50keywords, ROC, key2article, commongen")
...
if __name__ == "__main__":
    ...
    for j, keyword_set in enumerate(keyword_sets):
        if args.n_generated_sentences < 0:
            in_text, keywords = keyword_set
            n_generated_sentences = math.ceil((len(keywords) + 1) * abs(args.n_generated_sentences) / args.n_words_per_sentence)  # NOQA
        else:
