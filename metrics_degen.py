...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--n_sample", type=int, default=50, help="how many sentences to sample to calculate bleu")
...
def bleu_i(weights, all_sentences, smoothing_function, i):
    ...
    return sentence_bleu(references=all_sentences[:i] + all_sentences[i + 1 :], hypothesis=all_sentences[i], weights=weights, smoothing_function=smoothing_function)
...
def main():
    ...
    with open(args.file, "r") as file1:
        ...
        for line in tqdm(lines):
            ...
            if "Success_rate:" in line:
                ...
                # PPL:
                ppl[i] = distilGPT2_perplexity_score(stripped)  # NOQA
                i = i + 1
                text = ""
            elif next_yes:
                text = text + line
    ...
    # Distinct n 2
    print("distinct 2-grams\ttotal 2-grams\tdistinct proportion")
    text_file.write("distinct 2-grams\ttotal 2-grams\tdistinct proportion\n")
    print(f"{n_distinct_2}\t{n_total_2}\t{n_distinct_2/n_total_2}")
    text_file.write(f"{n_distinct_2}\t{n_total_2}\t{n_distinct_2/n_total_2}\n\n")

    # Distinct n 3
    print("distinct 3-grams\ttotal 3-grams\tdistinct proportion")
    text_file.write("distinct 3-grams\ttotal 3-grams\tdistinct proportion\n")
    print(f"{n_distinct_3}\t{n_total_3}\t{n_distinct_3/n_total_3}")
    text_file.write(f"{n_distinct_3}\t{n_total_3}\t{n_distinct_3/n_total_3}\n\n")

    # Distinct n 4
    print("distinct 4-grams\ttotal 4-grams\tdistinct proportion")
    text_file.write("distinct 4-grams\ttotal 4-grams\tdistinct proportion\n")
